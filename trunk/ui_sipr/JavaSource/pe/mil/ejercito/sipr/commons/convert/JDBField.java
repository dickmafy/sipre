...
public class JDBField {
  ...
  public JDBField(String s, char c, int i, int j) throws JDBFException {
    ...
    if (c == 'C' && i >= 255) {
      throw new JDBFException("CHAR O STRING -The field length should be less than 255 characters for character fields. Got: " +
                              i);
    }
    if (c == 'N' && i >= 21) {
      throw new JDBFException(
          "NUMBER - The field length should be less than 21 digits for numeric fields. Got: " +
          i);
    }
    if (c == 'L' && i != 1) {
      throw new JDBFException(
          "LOGICAL - The field length should be 1 characater for logical fields. Got: " +
          i);
    }
    if (c == 'D' && i != 8) {
      throw new JDBFException(
